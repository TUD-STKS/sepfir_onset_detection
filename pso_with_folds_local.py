...
def optimize_folds(folds: list, training_folds: list, test_fold: list,
    ...
    options = {'c1': 0.5, 'c2': 0.3,
               'w': 0.9, 'k': 3, 'p': 2}  # Set-up hyperparameters
    ...
    # Test on Test set
    # Preprocess data on remaining fold
    spec_list = []
    annotation_list = []
...
def main(audiodir: str = "data/audio", refdir: str = "data/annotations/onsets",
    ...
    logging.info(msg=f"#######################################################"
                     f"########################################")
    logging.info(msg=f"Started new Local optimization run! {num_particles} "
...
if __name__ == "__main__":
    ...
    p.add_argument(
        '-num_iterations', '--num_iterations', type=int, default=200,
